...
public abstract class StructureLogic implements IStructureLogic {
	...
	public void validateStructure() {
		...
		if (state == EnumStructureState.VALID) {
			...
			//System.out.println(String.format("Structure is valid at %s/%s/%s", ((TileEntity)structure).xCoord, ((TileEntity)structure).yCoord, ((TileEntity)structure).zCoord));
			...
			/*
			int offsetX = schematas[activeSchemata].getxOffset();
			int offsetZ = schematas[activeSchemata].getzOffset();
			if(isRotated) {
				offsetX = schematas[activeSchemata].getzOffset();
				offsetZ = schematas[activeSchemata].getxOffset();
			...
			for(int j = 0; j < schemata.getHeight(); j++)
				for(int k = 0; k < dimensions.z; k++) {
					...
					if(metaOnValid.containsKey(type)) {
						structureTile.worldObj.setBlockMetadataWithNotify(x, y, z, metaOnValid.get(type));
