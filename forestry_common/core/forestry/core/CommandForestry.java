...
public class CommandForestry extends CommandMC {
	...
	public void processCommand(ICommandSender sender, String[] arguments) {
		...
		if (arguments[0].matches("version")) {
			...
			sender.sendChatToPlayer("- plugins : list of Forestry plugins.");
			sender.sendChatToPlayer("- plugins info <plugin-name>: information on plugin.");
			return;
		}
	...
	private void commandVersion(ICommandSender sender, String[] arguments) {
		...
		sender.sendChatToPlayer(String.format(colour + "Forestry %s for Minecraft %s (Latest: %s).", Version.getVersion(),
	...
	private void listPluginsForSender(ICommandSender sender) {
		...
		sender.sendChatToPlayer(pluginList);
	}
	...
	private void listPluginInfoForSender(ICommandSender sender, String[] arguments) {
		...
		if (info != null) {
			sender.sendChatToPlayer(entry + "Plugin: " + info.name());
			if (!info.version().isEmpty()) {
				sender.sendChatToPlayer("\u00A79Version: " + info.version());
			}
			if (!info.author().isEmpty()) {
				sender.sendChatToPlayer("\u00A79Author(s): " + info.author());
			}
			if (!info.url().isEmpty()) {
				sender.sendChatToPlayer("\u00A79URL: " + info.url());
			}
			if (!info.description().isEmpty()) {
				sender.sendChatToPlayer(info.description());
			}
		}
