...
public class ContainerSolderingIron extends ContainerForestry implements IGuiSelectable {
	...
	public ContainerSolderingIron(InventoryPlayer inventoryplayer, SolderingInventory inventory) {
		...
		// Input
		this.addSlot(new SlotCustom(inventory, new Object[] { ItemCircuitBoard.class }, 0, 152, 12));

		// Output
		this.addSlot(new SlotCustom(inventory, new Object[] { ItemCircuitBoard.class }, 1, 152, 92));
		...
		for (int i1 = 0; i1 < 3; i1++) {
			for (int l1 = 0; l1 < 9; l1++) {
				addSlot(new Slot(inventoryplayer, l1 + i1 * 9 + 9, 8 + l1 * 18, 123 + i1 * 18));
			}
		}
		// Player hotbar
		for (int j1 = 0; j1 < 9; j1++) {
			addSlot(new Slot(inventoryplayer, j1, 8 + j1 * 18, 181));
		}
