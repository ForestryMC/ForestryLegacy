...
public class GrowthProvider implements IGrowthProvider {
	...
	/*
	protected EnumGrowthConditions getConditionFromGround(World world, int xPos, int yPos, int zPos) {
		Block block = Block.blocksList[world.getBlockId(xPos, yPos - 1, zPos)];
		if(block == null)
			return EnumGrowthConditions.HOSTILE;
		
		for(EnumPlantType type : this.validPlantTypes) {
			this.plantType = type;
			if(block.canSustainPlant(world, xPos, yPos - 1, zPos, ForgeDirection.UP, this))
				return EnumGrowthConditions.EXCELLENT;
		}
			...
			for (int y = (int) start.y; y < (int) start.y + area.y; y++) {
				for (int z = (int) start.z; z < (int) start.z + area.z; z++) {
					if (!world.isAirBlock(x, y, z)
							&& !Utils.isReplaceableBlock(world, x, y, z))
						return false;
				}
			}
		}
