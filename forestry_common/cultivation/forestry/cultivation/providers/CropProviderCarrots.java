...
public class CropProviderCarrots implements ICropProvider {
	...
	public boolean isGermling(ItemStack germling) {
		return germling.itemID == Item.carrot.itemID;
	}

	@Override
	public boolean isCrop(World world, int x, int y, int z) {
		int blockid = world.getBlockId(x, y, z);
		return blockid == Block.carrot.blockID;
	}
	...
	public boolean doPlant(ItemStack germling, World world, int x, int y, int z) {
		...
		world.setBlockAndMetadataWithNotify(x, y, z, Block.carrot.blockID, 0);
		return true;
	}

	@Override
	public ICropEntity getCrop(World world, int x, int y, int z) {
		return new CropCarrot(world, x, y, z);
	}
