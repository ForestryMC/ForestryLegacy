...
public class TradeStation extends WorldSavedData implements ILetterHandler, IInventory {
	...
	private GenericInventoryAdapter inventory = new GenericInventoryAdapter(SLOT_SIZE, "INV");
	...
	public TradeStationInfo getTradeInfo() {
		...
		return new TradeStationInfo(this.moniker, inventory.getStackInSlot(SLOT_TRADEGOOD), condensedRequired, state);
	}

	// / ILETTERHANDLER
	@Override
	public IPostalState handleLetter(World world, MailAddress address, ItemStack letterstack, boolean doLodge) {
		...
		ILetter mail = new Letter(new MailAddress(this.moniker, EnumAddressee.TRADER), letter.getSender());
		...
		for (int i = 0; i < stampCount.length; i++) {
			if (stampCount[i] > 0) {
				mail.addStamps(new ItemStack(ForestryItem.stamps, stampCount[i], EnumPostage.values()[i].ordinal()));
			}
		}
		...
		ItemStack mailstack = new ItemStack(ForestryItem.letters, 1, ItemLetter.encodeMeta(1, ItemLetter.getType(mail)));
		...
		if (!responseState.isOk()) {
			System.out.println("ResponseState: " + responseState.getIdentifier());
		...
		if (doLodge && owner != null && !owner.isEmpty()) {
			ILetter confirm = new Letter(new MailAddress(this.moniker, EnumAddressee.TRADER), new MailAddress(this.owner));
			confirm.setText(ordersToFill + " order(s) from " + letter.getSender().getIdentifier() + " were filled.");
			confirm.addStamps(new ItemStack(ForestryItem.stamps, 1, EnumPostage.P_1.ordinal()));
			nbttagcompound = new NBTTagCompound();
			confirm.writeToNBT(nbttagcompound);
			ItemStack confirmstack = new ItemStack(ForestryItem.letters, 1, ItemLetter.encodeMeta(1, ItemLetter.getType(confirm)));
	...
	private int countFillableOrders(int max, ItemStack tradegood) {
		...
		for (ItemStack stack : inventory.getStacks(SLOT_INPUTBUF_1, SLOT_BUFFER_COUNT))
			if (stack != null && stack.isItemEqual(tradegood)
				&& ItemStack.areItemStackTagsEqual(stack, tradegood)) {
	...
	private int countStorablePayment(int max, ItemStack[] exchange) {
		...
		GenericInventoryAdapter test = inventory.copy();
		int count = 0;
