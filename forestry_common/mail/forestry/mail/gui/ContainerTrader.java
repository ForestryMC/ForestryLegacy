...
public class ContainerTrader extends ContainerForestry {
	...
	public ContainerTrader(InventoryPlayer player, MachineTrader tile) {
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 2; j++) {
				addSlot(new SlotCustom(inventory, new Object[] { ItemStamps.class }, TradeStation.SLOT_STAMPS_1 + j + i * 2, 15 + j * 18, 37 + i * 18));
			}
		}
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 2; j++) {
				addSlot(new SlotCustom(inventory, new Object[] { Item.paper }, TradeStation.SLOT_LETTERS_1 + j + i * 2, 15 + j * 18, 73 + i * 18));
			}
		}
